...
if [ "$1" != "--package" ]; then
  echo "Error: expected --package as the first parameter"
  exit 1
fi
pkg="$2"
shift 2
...
if [ "$1" != "--keystore" ]; then
  echo "Error: expected --keystore as the third parameter"
  exit 1
fi
keystore="$2"
shift 2
...
cert=$(keytool -list -rfc -keystore "$keystore" | sed -e '1,/BEGIN/d' | sed -e '/END/,$d' | tr -d "[:space:]" | base64 --decode | xxd -p | tr -d ' \n')
...
output=$(printf "$input" | sha256sum | cut -c1-64)
echo
